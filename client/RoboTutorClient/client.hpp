	...
	class Client : public Connection<ProtocolT, false> {
		...
		public:
			...
			Client(typename BaseType::must_be_shared_ must_be_shared, boost::asio::io_service & ios) :
				BaseType(must_be_shared, std::make_shared<typename Protocol::Transport::socket>(ios)),
				extension_(*this) {}
			 ...
			 */
			std::enable_if<std::is_same<typename Protocol::Transport, boost::asio::ip::tcp>::value>
			connectIp4(boost::asio::ip::address_v4::bytes_type const & address, unsigned short port) {
			 ...
			 */
			std::enable_if<std::is_same<typename Protocol::Transport, boost::asio::ip::tcp>::value>
			connectIp4(std::string const & address, unsigned short port) {
			 ...
			 */
			std::enable_if<std::is_same<typename Protocol::Transport, boost::asio::ip::tcp>::value>
			connectIp6(boost::asio::ip::address_v6::bytes_type const & address, unsigned short port) {
			 ...
			 */
			std::enable_if<std::is_same<typename Protocol::Transport, boost::asio::ip::tcp>::value>
			connectIp6(std::string const & address, unsigned short port) {
		...
		protected:
			 ...
			 */
			void handleMessage_(typename Protocol::ServerMessage const & message) {
				if (extension_.handleMessage(message) && message_handler) {
					message_handler(get_shared_(), message);
				}
			}
