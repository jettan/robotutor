...
namespace robotutor {
	namespace command {
		...
		Behavior::Behavior(ArgList && arguments) {
			if (arguments.size() != 1) throw std::runtime_error("Wrong amount of arguments for behavior command. Expected 1.");
			command::Text::SharedPtr text = std::dynamic_pointer_cast<command::Text const>(arguments[0]);
			behavior = text->original;
			if (!behavior.size()) throw std::runtime_error("Empty behavior name not allowed for behavior command.");
		}
